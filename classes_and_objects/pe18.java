...
class pe18 {
    ...
    public static void main(String args[]) throws Exception {
        ...
        while(true){
            BigInteger val = getFactorial(start);
            int max_bit_len = BigInteger.valueOf(Long.MAX_VALUE).bitLength();
            if(val.bitLength() > max_bit_len ){
                System.out.println(start + " factorial : Out of range");
                // System.out.println(BigInteger.valueOf(Long.MAX_VALUE).bitLength());
                // System.out.println(getFactorial(start).bitLength());
                break;
            }
            else{
